...
loading = waitbar(0, 'Processing image depending on structure...');
...
for i = 1:i_range
    for j = 1:j_range
        ...
        % ----------- Find the three smallest differences -----------
        [~, sorted_indices] = sort(differences);
        best_indices(i, j, :) = sorted_indices(1:3);

        % ----------- Compare the three best images right away -----------
        best_images = imgs(sorted_indices(1:3));
        ssim_values = zeros(3, 1);

        for idx = 1:3
            ssim_values(idx) = ssim(area, best_images{idx});
        end

        % ----------- Choose the best image based on SSIM -----------
        [~, best_idx] = max(ssim_values);
        res_img((i-1)*checking_size+1 : i*checking_size, (j-1)*checking_size+1 : j*checking_size, :) = best_images{best_idx};
    end
